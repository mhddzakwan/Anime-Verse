...
class AnimeRepository {
  ...
  /// Mendapatkan daftar anime teratas
  Future<List<Anime>> getTopAnime({int page = 1}) async {
    try {
      final url = Uri.parse('$_baseUrl/top/anime')
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Rate limit exceed. Please wait a moment and try again.');
      } else if (response.statusCode == 404) {
        throw Exception('Top Anime data not found');
      } else {
        throw Exception(
            'Failed to load top anime. Status: ${response.statusCode}');
      }
    } catch (e) {
      if (e is Exception) {
        rethrow;
  ...
  /// Dispose HTTP client ketika repository tidak lagi digunakan
  void dispose() {
    _client.close();
  }
}
